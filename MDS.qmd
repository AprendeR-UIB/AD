...
De una manera equivalente, se puede obtener la matriz $\mathbf{X}$ a partir de la matriz $\mathbf{Q}$. Si ésta es definida positiva de rango $p$ entonces puede expresarse por
$$\mathbf{Q}=\mathbf{V} \mathbf{\Lambda}\mathbf{V}^t$$
donde $\mathbf{V}$ es una matriz $n \times p$ y $\mathbf{\Lambda}$ es una matriz diagonal $p \times p$ y contiene los vectores propios no nulos de $\mathbf{Q}$. Entonces, podemos escribir
...
se obtienen $p$ variables incorreladas que reproducen la métrica.
