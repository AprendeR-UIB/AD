...
1. No tiene unidades (porque las unidades de $\sigma_X$ son las de $X$, las unidades de $\sigma_*Y$ son las de $Y$, y las unidades de $\sigma_{X,Y}$ son las de $X$ por las de $Y$)
...
X1=X[-101]
X2.menos.X1=diff(X)
plot(X1,X2.menos.X1,pch=20,xlab=expression(X[1]),ylab=expression(X[2]-X[1]))
cor(X1,X2.menos.X1)
```
...
* Cuando $\widetilde{S}_{X,Y}>0$, si $x_y>x_j$ entonces $y_y$ tiende a ser más grande que $y_j$

* Cuando $\widetilde{S}_{X,Y}<0$, si $x_y>x_j$ entonces $y_y$ tiende a ser más pequeño que $y_j$
...
Hemos mesurado el índice de masa corporal, BMI, y el nivel de colesterol en 5 individuos sanos. Guardamos los resultados en un *dataframe* y calculamos las medias:
...
Es conveniente acompañar el cálculo de correlación o covarianza de dos vectores $x,y$ con un gráfico de los puntos $(x_y,y_y)$, porque conjuntos muy diferentes de puntos pueden dar lugar a la misma correlación.
...
* La covarianza $S_{X,Y}$ es el estimador máximo verosímil de $\sigma_{X,Y}$ cuando la distribución conjunta de las variables $X,Y$ es la [normal bivariante](http://es.wikipedia.org/wiki/distribución_normal_multivariante}) que estudiaremos próximamente.
...
x_i>x_j\Longleftrightarrow y_i<y_j
$$
Cuanto más se acerca la correlación de Spearman a valer 1 (o -1), para más parejas de índices $(y,j)$ tales que $x_y>x_j$ se tiene que $y_y>y_j$ ($y_y<y_j$, si se acerca a -1).
...
* Como que solo hay un 3 a $x$, su rango es su posición en el vector ordenado: 3.
...
Por tanto, la correlacióo de Spearman de 
$$
...
*Conclusión*: No hemos obtenido evidencia estadísticamente significativa que el BMI y el nivel de colesterol de un adulto sano *tengan correlación positiva (test de correlación, p-valor 0.06, IC 95% para $\rho$ de -0.086 a 1).
...
<!-- ## Contrastes de hipótesis univariantes versus multivariantes -->